...
/// <summary>
/// MotionClock = tüm hareketlerin ortak saati.
/// - Server-authoritative: Start/Pause/Resume/Reset sadece server yazar; NetVar'larla client'lara yayılır.
/// - Driver script'leri her kare EffectiveTime (ET) okur ve hareketi LOKAL uygular (NetworkTransform kullanma).
...
public class MotionClock : NetworkBehaviour
{
    // -------- Inspector --------
    [Header("Startup")]
    ...
    public double EffectiveTime
    {
        get
        {
            ...
            // global scale (driver kendi scale'ini ayrıca çarpabilir)
            return et * Mathf.Max(0f, TimeScale);
        }
    }

    /// <summary>
    /// ET'nin scale uygulanmamış (ham) değeri gerekir ise.
    /// </summary>
    ...
    public void Resume()
    {
        ...
        isPausedNV.Value    = false;
        pausedStartServer   = 0.0;
        // snapshot tutulsa da paused değilken kullanılmıyor
    }
    ...
    public override void OnNetworkSpawn()
    {
        ...
        // (İsteğe bağlı) NetVar değişimlerinde event basmak istersen, buralara OnValueChanged ekleyebilirsin.
        // isPausedNV.OnValueChanged += (oldV, newV) => { ... };
    }
